...
public class Collector {
    ...
    private boolean isDuplicateInSameHtml(List<HttpMessage> list, HttpMessage msg) {
	...
	void collect(Html html, int currentDepth) {
	    List<HttpMessage> previousFoundList = new ArrayList<HttpMessage>();
...
//	    }
	    ...
	    List<HttpMessage> formQueryList = getFormsQuery(html);
	...
	public List<HttpMessage> getFormsQuery(Html html) {
		List<HttpMessage> qryList = new Vector<HttpMessage>();
		Form[] forms = html.getForms();
		for (int i=0; i<forms.length; i++) {
			Form form = forms[i];
			List<HttpMessage> oneForm = getFormQuery(form, html.getURI());
			qryList.addAll(oneForm);
		}
		return qryList;
	}

	private List<HttpMessage> getFormQuery(Form form, URI baseURI) {
		List<String> qryStrList	= new Vector<String>();
		List<HttpMessage> qryList = new Vector<HttpMessage>();
		String queryString = "";
		...
		try {
			...
			for (int i=0; i<qryStrList.size(); i++) {
			    HttpMessage msg = null;
				URI uri = null;
				String qryStr = qryStrList.get(i);
	...
	private List<String> addSelectField(List<String> qry, Select select) {
		List<String> newQryList = new Vector<String>();
		...
		for (int i=0; i<select.getOption().length && i<2; i++) {
            ...
            try {
				if (qry.isEmpty()) {
					String queryString = buildPostQueryString("", select.getName(), value);
					newQryList.add(queryString);
				} else {
					for ( String queryString : qry ) {
