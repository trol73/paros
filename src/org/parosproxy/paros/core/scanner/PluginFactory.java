...
public class PluginFactory {
    ...
    private static Map<Integer, Plugin> mapAllPlugin = new TreeMap<Integer, Plugin>();
    private static Map<String, Plugin> mapAllPluginOrderCodeName = new TreeMap<String, Plugin>();
    ...
    public PluginFactory() {
        ...
        synchronized (mapAllPlugin) {
        	...
        	for ( Plugin plugin : mapAllPlugin.values() ) {
                if ( plugin.isEnabled() ) {
    ...
    private void enableDependency(Plugin plugin) {
        ...
        for (int i=0; i<dependency.length; i++) {
            try {
                Plugin p = mapAllPluginOrderCodeName.get(dependency[i]);
                if ( p == null ) {
                	continue;
                }
                p.setEnabled(true);
...
//                plugin.setEnabled(true);
            ...
            listAllPlugin.addAll(mapAllPlugin.values());
        }
